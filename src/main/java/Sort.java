...
public class Sort {
        ...
        for (int i = 0; i < inputFileScanners.length; i++) {
            if (arrWithLastScannedValues[i] == null) {
                ...
                if (arrWithLastScannedValues[i].contains(" ")) {
                    throw new Exception("Недопустимы пробелы в исходном файле");
                }
            }
    ...
    private static int getValueDependsOnDataType(String line) throws Exception {
        if (dataType == DataType.INTEGERS) {
            try {
                ...
                throw new Exception("Невозможно преобразовать string '" + line + "' в int");
            }
        } else {
            return line.length();
        }
    ...
    // Эта проверка позволяет обрабатывать случай, когда входные файлы не отсортированы.
